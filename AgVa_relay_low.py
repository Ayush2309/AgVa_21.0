...
#-------------------------------------------
    ...
    def read_data(self):
	global initial_pump_pressure, motor_factor,change_setting,primitive_inhale_time,compliance_array, compliance_flag,toggle_switch,peep_open,setting_set,peep_pwm,peep_array,P_plat_high,P_plat_low,flow_plat,previous_data,pump_pressure_low,pump_pressure_high, mode_set,inhale_time,P_plat_value,P_plat_array,peep_val,prev_mode,pump_pressure_array,BPM,PIP,VTi_max,peep,P_plat,trigflow,cycle_time,data,pump_pressure
	try:
...
#		    print(pump_pressure)
		...
		if(P_plat_low <= 0):
		    P_plat_low = 0
   	   # print("peeeepppppppppppppppppp i have got is")
		previous_data = data
	    #print(peep)
#	    print(BPM)
	        cycle_time=60.0/BPM
...
#	    motor_1.ChangeDutyCycle(40)
    ...
    def Flow(self):
        ...
        global volume,clock_t2,P_plat,flow_error_compensation,indiff,inhale_loop
...
#			exp_flow_array.append(exp_flow)
			...
			ratio = peep_val/peak_insp_pressure
...
#		    print("SDP                  " + str(diff))
		    ...
		    send_last_time = 0
		    sol_flag = 0
                    while((time_elapsed_exhale<((cycle_time-inhale_time)-0.4) and (diff <= 0)) or (SDP_flag ==1 and time_elapsed_exhale<(cycle_time-inhale_time-0.5))):
			exp_flow = self.rate()
			print("Time elapsed exhale  " + str(time_elapsed_exhale))
#			print("ABP                  " + str(indiff))
#			print("SDP                  " + str(diff))
...
#		        print(MVi)
		        MVi_array.pop(0)
		        packet_end_inhalation = ('B@' + str(round(peak_insp_pressure,2)) + "," + str(int(VTi_volume - (VTi_volume*leak_percentage)/100)) + "," + str(round(volume_peak_inhale,2)) + "," + str(round(Pmean,2)) + "," + str(round((MVi/1000),2)) + "," + str('0') + "," + str(round(time_elapsed_inhale,2)) + "," + str(round(indiff,2)) + "," + str('0') + "," + str(round(rise_time,2)) + '#')
#		        print('packet length is')
		        print(packet_end_inhalation)
		        try:
		            ser.write(packet_end_inhalation)
...
#		    print('volume is')
		    indiff = int(indiff)
		    if((VTi_volume >= VTi_max or  indiff >= P_plat ) and time_elapsed_inhale < (inhale_time * 0.8)  and ABP_flag == 0 and compliance_flag == 1  and SDP_flag == 0 and patient_status == 1 and pump_pressure >= 20 and pump_pressure <= 99):
			pump_pressure = pump_pressure - 1
			P_plat = indiff
#			print("--------------------------------")
		    elif((VTi_volume < VTi_max or indiff < (P_plat - 1)) and compliance_flag == 1  and ABP_flag == 0 and SDP_flag == 0 and indiff <= PIP and patient_status == 1 and pump_pressure >= 19 and pump_pressure <= 95):
			if((VTi_max - VTi_volume) > 100 and leak_percentage < 70 and VTi_max > 100 ):
...
#		        peep_val_send =  exp_press_array[len(exp_press_array) - 3]
			...
			if(toggle_switch == "1"):
			    ...
			    peep_open = peep_open - 1
		    print("peep pwm is:::::::::::::::::::::::::::::::::: "  + str(peep_open))
		    if(int(peep_val_send) > peep_val+2):
			peep_count = peep_count + 1
			if(leak_percentage > 20 and trigger == '0'):
			    ser.write("ACK39")
...
#		    print(pump_pressure_high)
		    indiff = int(indiff)
		    if(VTi_volume > VTi_max and ABP_flag == 0 and time_elapsed_inhale < (inhale_time * 0.8) and SDP_flag == 0 and patient_status == 1 and pump_pressure >= 20 and pump_pressure <= 99):
			pump_pressure = pump_pressure - 1
			P_plat = indiff
			print('-1')
		    elif(VTi_volume < VTi_max and ABP_flag == 0 and SDP_flag == 0 and indiff <= PIP and patient_status ==1 and pump_pressure >= 19 and pump_pressure <= 95):
			if((VTi_max - VTi_volume) > 100 and leak_percentage < 70 and VTi_max > 100):
			    if(((VTi_max - VTi_volume)/3 ) > 5):
				pump_pressure = pump_pressure + 5
			    else:
...
#		    flow_zero_elapsed = 0
		    peak_flow = volFlow_rate
		    rise_flag = '0'
                    while(((peak_flow*0.10 <= volFlow_rate) and time_elapsed_inhale <= inhale_time) or time_elapsed_inhale <= 0.5):
                    #    motor_1.ChangeDutyCycle(pump_pressure)
                        #motor_2.ChangeDutyCycle(pump_pressure)
                        t3 = time()
			GPIO.output(37, GPIO.HIGH)
...
#		    motor_1.ChangeDutyCycle(peep_open)
		 ...
		 #   GPIO.output(35, GPIO.HIGH) # HIGH level triggered relay
		    ...
		    print(P_plat)
                    RR.append(RR_time)
		    peak_insp_pressure = int(peak_insp_pressure)
		    if(peak_insp_pressure > P_plat and patient_status == 1 and ABP_flag == 0 and peak_insp_pressure < PIP  and pump_pressure >= 1 and pump_pressure <= 96):
			pump_pressure = pump_pressure - 1
			print('-1')
		    elif(peak_insp_pressure < P_plat and patient_status == 1 and ABP_flag == 0 and peak_insp_pressure < PIP and  pump_pressure >= 0 and pump_pressure <= 95):
...
#		    print(Pmean)
		...
		if(flag == 1):
		    patient_set = 0
		    if((peak_insp_pressure <= P_plat * 0.4)  and patient_set == 0 ):
			buzzer(5,1)
			patient_set = 1
			patient_status = 0
		        print('disconnection')
...
#		        print(MVi)
		        MVi_array.pop(0)
		        packet_end_inhalation = ('B@' + str(round(peak_insp_pressure,2)) + "," + str(int(VTi_volume - (VTi_volume*leak_percentage)/100)) + "," + str(round(volume_peak_inhale,2)) + "," + str(int(Pmean)) + "," + str(round((MVi/1000),2)) + "," + str(round(patient_trigger_flow,2)) + "," + str(round(time_elapsed_inhale,2)) + "," + str(round(indiff,2)) + "," + str('0') + "," + str(round(rise_time,2)) +'#')
#		        print('packet length is')
#		        print(packet_end_inhalation)
		        try:
		            ser.write(packet_end_inhalation)
...
#-----------------------------------------------------------
    def NIV_BIPAP(self):
        global initial_pump_pressure, exp_press,exp_press_list,peep_average,peep,peep_open,time_elapsed_exhale_flow,patient_trigger_flow,rise_time, rise_flag ,pressure_low_count,BIPAP_backup_flag,back_backup,inhale_loop,toggle_switch,loop,peep,patient_set,ABP_flag,exhale_time_last,TOT_last,TITOT,pressure_count,peep_count, SDP_flag,flow_plat,indiff,patient_status,pump_pressure,pump_pressure_low,pump_pressure_high,ratio_set,time_error_set,thread_mode_status,time_error, time_error_last,ABP_flag,P_plat_value,prev_mode,data,ratio,RR_time,indiff,peep_val, peep_hole, RR, BPM, MVi_array, MVi,volume, volume_peak_inhale, trigger, max_value, volume_peak_exhale, MVe_array, MVe,clock_t2
        thread_mode_status = True
	time_elapsed_exhale_flow = 0
	rise_time = 0
	patient_trigger_flow = 0
...
#		    flow_zero_flag = 0
		    ...
		    peak_flow = volFlow_rate
		    rise_flag = '0'
                    while((((peak_flow*0.40 <= volFlow_rate) and time_elapsed_inhale <= inhale_time) or time_elapsed_inhale <= 0.3) and indiff < P_plat * 1.3 ):
...
#		    print(pump_pressure_high)
		    indiff = int(indiff)
		    if(VTi_volume > VTi_max  and compliance_flag == 1 and time_elapsed_inhale < (inhale_time * 0.8) and ABP_flag == 0 and SDP_flag == 0 and patient_status == 1 and pump_pressure >= 10 and pump_pressure <= 95):
			pump_pressure = pump_pressure - 1
			P_plat = indiff
#			print('-1')
		    elif(VTi_volume < VTi_max and ABP_flag == 0 and compliance_flag == 1 and SDP_flag == 0 and indiff <= PIP and patient_status ==1 and pump_pressure >= 10 and pump_pressure <= 95):
			if((VTi_max - VTi_volume) > 100 and leak_percentage < 70  and VTi_max > 100):
			    if(((VTi_max - VTi_volume)/3 ) > 5):
				pump_pressure = pump_pressure + 5
			    else:
...
#		print(flow)
		...
		print(current_time - start_time)
		loop = 1
                if(flow>trig_flow or ((current_time-start_time)>cycle_time and backup_flag == 1)):
		    RR_time=current_time-start_time
		    pump_pressure_now = pump_pressure
		    volume = 0
		    inhale_loop = "1"
...
#		        print(packet_end_inhalation)
		    ...
		    send_last_time = 0
		    volume_peak_exhale = 0
		    while(time_elap < 0.01 or (toggle_switch == "1" and time_elap < 0.4)):
			w=time()
			time_elap=w-q
                        indiff = self.ABP_pressure()
			ratio = peep_val/P_plat
...
#		    print(clock_t2)
		    ...
		    peak_flow = volFlow_rate
		    rise_flag = '0'
                    while(indiff <= PIP and time_elapsed_inhale <= inhale_time and ((volume_comp <= VTi_max * 1.05 ) or trigger == '1')):
                    #    motor_1.ChangeDutyCycle(pump_pressure)
                        #motor_2.ChangeDutyCycle(pump_pressure)
			GPIO.output(37, GPIO.HIGH)
                        t3 = time()
...
#		    print(pump_pressure_high)
		    indiff = int(indiff)
		    if(VTi_volume > VTi_max  and compliance_flag == 1 and ABP_flag == 0 and SDP_flag == 0 and patient_status == 1 and pump_pressure >= 10 and pump_pressure <= 99):
			pump_pressure = pump_pressure - 1
			P_plat = indiff
			print('-1')
		    elif(VTi_volume < VTi_max and ABP_flag == 0 and compliance_flag == 1 and SDP_flag == 0 and indiff <= PIP and patient_status ==1 and pump_pressure >= 10 and pump_pressure <= 95):
...
#		    print(clock_t2)
		    ...
		    peak_flow = volFlow_rate
		    rise_flag = '0'
                    while(((volume_comp <= VTi_max * 1.05 ) or (volume_comp >= VTi_max and volFlow_rate > peak_flow * 0.40 and trigger == '1')) and indiff < PIP and time_elapsed_inhale <= inhale_time):
                    #    motor_1.ChangeDutyCycle(pump_pressure)
                        #motor_2.ChangeDutyCycle(pump_pressure)
			GPIO.output(37, GPIO.HIGH)
                        t3 = time()
...
#		    print(pump_pressure_high)
			...
			P_plat = indiff
#			print('-1')
		    elif(VTi_volume < VTi_max and ABP_flag == 0 and SDP_flag == 0 and indiff <= PIP and patient_status ==1 and pump_pressure >= 10 and pump_pressure <= 95):
			if((VTi_max - VTi_volume) > 100 and leak_percentage < 70 and VTi_max > 100):
			    if(((VTi_max - VTi_volume)/3 ) > 5):
				pump_pressure = pump_pressure + 5
			    else:
